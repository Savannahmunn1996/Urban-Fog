  ...
  fetch(requestUrl)
    ...
    .then(function (data) {
      ...
      const eleArr = mapArr.map(item => {
        return `
        <div class="day-card">
        <img class="the-icon" src="images/windy.png">
        <div>
...
function parseArray(data) {
  ...
  while(ans.length < 6 || i < data.length) {
    if(data[i].dt_txt.slice(0, 10) !== ans[ans.length - 1].dt_txt.slice(0, 10) && data[i].dt_txt.slice(11, data[i].dt_txt.length) == "12:00:00") {
      ans.push(data[i]);
    }
    i++;
  }
